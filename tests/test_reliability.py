...
def test_recovers_from_transient_rate_limits(stub):
    ...
    summary = reliability.call_with_retry(
        summarizer.summarize_notes, notes, retries=4, backoff_seconds=0
    )
...
def test_persistent_failure_raises_upstream_error(stub):
    ...
    with pytest.raises(reliability.UpstreamError):
        reliability.call_with_retry(
            summarizer.summarize_notes, notes, retries=2, backoff_seconds=0
        )
...
def test_transport_errors_are_retried(stub):
    ...
    req = httpx2.Request("POST", "https://example.test")
    ...
    def flaky_transport():
        attempts["count"] += 1
        if attempts["count"] < 3:
            raise APIConnectionError(message="connection dropped", request=req)
        return "recovered"

    assert (
        reliability.call_with_retry(flaky_transport, retries=3, backoff_seconds=0)
        == "recovered"
    )
